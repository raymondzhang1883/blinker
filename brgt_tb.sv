...
module brgt_tb;
  ...
  initial begin
    ...
    uut.memory.bytes[8193] = 8'b01000000;  // rt= 4
    uut.memory.bytes[8194] = 8'b10000110;  // rd = 2, rs = 3
    uut.memory.bytes[8195] = 8'b01110000;  // opcode=01110  

    //addi r0, 10
    uut.memory.bytes[8196] = 8'b00001010; // literal=10
    uut.memory.bytes[8197] = 8'b00000000; // 
    uut.memory.bytes[8198] = 8'b00000000; // rd=0
    uut.memory.bytes[8199] = 8'b11001000; // opcode=11001 (0x19)

    //addi r0, 1
    uut.memory.bytes[8200] = 8'b00000001; // literal=1
    uut.memory.bytes[8201] = 8'b00000000; // 
    uut.memory.bytes[8202] = 8'b00000000; // rd=0
    uut.memory.bytes[8203] = 8'b11001000; // opcode=11001 (0x19)

    //halt
    uut.memory.bytes[8204] = 8'h00; // literal=0
    uut.memory.bytes[8205] = 8'h00; // rt=0
    uut.memory.bytes[8206] = 8'h00; // rs=0, rd=0
    uut.memory.bytes[8207] = 8'h78; // opcode=01111 (0x78)

    // Display memory contents after loading
    $display("LOADED MEMORY");
    $display("1. BR r2 (Branch to address in r2): %h %h %h %h", 
             uut.memory.bytes[8195], 
             uut.memory.bytes[8194], 
             uut.memory.bytes[8193], 
             uut.memory.bytes[8192]);
    $display("2. ADDI r0, 10 (at 0x8200): %h %h %h %h", 
             uut.memory.bytes[8199], 
             uut.memory.bytes[8198], 
             uut.memory.bytes[8197], 
             uut.memory.bytes[8196]);
    $display("3. ADDI r0, 1 (at 0x8204): %h %h %h %h", 
             uut.memory.bytes[8203], 
             uut.memory.bytes[8202], 
             uut.memory.bytes[8201], 
             uut.memory.bytes[8200]);
    $display("4. HALT instruction (at 0x8208): %h %h %h %h", 
             uut.memory.bytes[8207], 
             uut.memory.bytes[8206], 
             uut.memory.bytes[8205], 
             uut.memory.bytes[8204]);
    
    // Add VCD dump after memory initialization
    $dumpfile("brgt_tb.vcd");
    $dumpvars(0, brgt_tb);
    ...
    uut.reg_file.registers[0] = 0;       // Initialize r0 to 0
    uut.reg_file.registers[2] = 64'd8200;       // Initialize r2 to 0x8200 (33280)
    uut.reg_file.registers[3] = 64'd10;       // Initialize r3 to 0x8204 (33284)
    uut.reg_file.registers[4] = 64'd5;       // Initialize r4 to 0x8208 (33288)
    $display("Initial r0 = %d", uut.reg_file.registers[0]);
    $display("Initial r2 = 0x%h (%0d decimal)", uut.reg_file.registers[2], uut.reg_file.registers[2]);
    $display("Initial PC = 0x%h (%d decimal)", uut.pc, uut.pc);
    // Run until halt or timeout with longer timeout for debugging
    fork
      begin
        repeat (20) @(posedge clk); // Reduce this for quicker initial debugging
        $display("Simulation status after 20 cycles:");
        ...
        repeat (10) @(posedge clk); // Continue to full timeout
        $display("Simulation timeout after 30 cycles");
        $finish;
      end
      begin
        @(posedge hlt);
    ...
    if (uut.reg_file.registers[0] == 1) begin
      $display("SUCCESS: Register r0 successfully loaded with value 10");
    end else begin
